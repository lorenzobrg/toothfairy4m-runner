...
def _localize_inputs(
    *, storage: ObjectStorage, workdir: str, input_spec: Any
    ...
    def _extract_input_path(value: Any) -> str:
        if isinstance(value, dict):
            for candidate in ("path", "key", "file_path", "object_key"):
    ...
    if isinstance(input_spec, dict):
        ...
        if files_list is not None:
            for idx, v in enumerate(files_list):
                download_one(f"file_{idx}", _extract_input_path(v))
        else:
            for k, v in input_spec.items():
                if isinstance(v, list):
                    for idx, item in enumerate(v):
                        download_one(f"{k}_{idx}", _extract_input_path(item))
                else:
                    download_one(str(k), _extract_input_path(v))
    elif isinstance(input_spec, list):
        for idx, v in enumerate(input_spec):
            download_one(f"file_{idx}", _extract_input_path(v))
    else:
        download_one("input", _extract_input_path(input_spec))
