...
def _sanitize_output_key(key: str) -> str:
    normalized = (key or "").replace("\\", "/").lstrip("/")
    parts = [segment for segment in normalized.split("/") if segment not in {"", ".", ".."}]
    sanitized = "/".join(parts)
    if not sanitized:
        raise RunnerError("Invalid output object key")
    return sanitized
    ...
    try:
        ...
        for logical_name, out_spec in outputs.items():
            ...
            if isinstance(out_spec, str):
                ...
                if isinstance(out_spec.get("key"), str) and out_spec.get("key").strip():
                    key_override = out_spec.get("key").strip()
            else:
                continue
            ...
            rel_out_path = os.path.relpath(abs_out_path, output_dir).replace("\\", "/")
            if rel_out_path.startswith("../"):
                rel_out_path = os.path.basename(abs_out_path)
            rel_out_path = _sanitize_output_key(rel_out_path)

            if key_override:
                key = _sanitize_output_key(key_override)
            else:
                key = _sanitize_output_key(
                    f"{project_slug}/processed/{modality}/job_{job_id}/{rel_out_path}"
                )
            ...
            if not transcription_text and str(abs_out_path).lower().endswith(".txt"):
                try:
                    with open(abs_out_path, "r", encoding="utf-8", errors="replace") as txt_f:
                        candidate = txt_f.read().strip()
                    if candidate:
                        transcription_text = candidate
                except Exception:
                    logger.warning("Unable to read transcription output text from %s", abs_out_path)

        result_logs = logs
        if modality == "audio":
            # ToothFairy4M stores logs into text_caption for audio jobs.
            result_logs = transcription_text or ""

        return RunnerResult(outputs=uploaded_outputs, logs=result_logs)
    finally:
