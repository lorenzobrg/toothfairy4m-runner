    ...
    *, cfg: RunnerConfig, storage: ObjectStorage, claimed_job: Dict[str, Any]
    ...
    try:
        ...
        passthrough_defaults = [
            "IOS_ORIENTER_CHECKPOINT",
            "IOS_ORIENTER_NUM_POINTS_UPPER",
            "IOS_ORIENTER_NUM_POINTS_LOWER",
            "IOS_ORIENTER_SEED",
        ]
        for name in passthrough_defaults:
            value = os.getenv(name)
            if value:
                env[name] = value

        passthrough_extra = os.getenv("ALGORITHM_ENV_PASSTHROUGH", "")
        for name in [x.strip() for x in passthrough_extra.split(",") if x.strip()]:
            value = os.getenv(name)
            if value:
                env[name] = value
